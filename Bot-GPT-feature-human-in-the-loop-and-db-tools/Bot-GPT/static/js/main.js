...
let chatContainer, chatInput, sendButton, modelSelect, fileExplorer,
    ...
    hidePlanPanelFn, hideCanvasPanelFn;
...
async function initializeApp(username) {
    ...
    function showOverlay(context) {
        if (!overlay) return;
        if ((context === 'canvas' || context === 'plan') && window.innerWidth >= 1024) {
            return;
        }
        overlay.dataset.openContext = context;
        overlay.classList.remove('hidden');
        if (context === 'canvas' || context === 'plan') {
    ...
    function hideOverlay(context) {
        if (!overlay) return;
        if (overlay.dataset.openContext !== context) {
            return;
        }
        ...
        delete overlay.dataset.openContext;
    }
    ...
    function showPlanPanel() {
        ...
        if (window.innerWidth < 1024) {
            planPanel.classList.add('mobile-visible');
            hideCanvasPanel();
            showOverlay('plan');
        } else {
            planPanel.classList.remove('mobile-visible');
            hideOverlay('plan');
        }
    }
    ...
    function hidePlanPanel() {
        ...
        planPanel.classList.remove('flex', 'mobile-visible');
        hideOverlay('plan');
    }
    ...
    function handleResponsiveResize() {
        ...
        if (window.innerWidth >= 1024) {
            ...
            if (canvasPanelEl) {
                canvasPanelEl.classList.remove('mobile-visible');
                if (!canvasPanelEl.classList.contains('hidden') && resizerEl) {
            ...
            if (canvasPanelEl && !canvasPanelEl.classList.contains('hidden')) {
                canvasPanelEl.classList.add('mobile-visible');
                if (resizerEl) {
    ...
    if (!window.__botGptResizeListenerAdded) {
        window.addEventListener('resize', handleResponsiveResize);
        window.__botGptResizeListenerAdded = true;
    }
    ...
    menuBtn.addEventListener('click', () => {
        const isHidden = sidePanel.classList.contains('-translate-x-full');
        if (isHidden) {
            hideCanvasPanel();
            hidePlanPanel();
    ...
    overlay.addEventListener('click', () => {
        const context = overlay.dataset.openContext;
        if (context === 'sidebar') {
            sidePanel.classList.add('-translate-x-full');
            hideOverlay('sidebar');
    ...
    planToggleBtn.addEventListener('click', () => {
        if (planPanel.classList.contains('hidden')) {
            showPlanPanel();
        } else {
            hidePlanPanel();
        }
...
function attachFileEventListeners() {
    document.querySelectorAll('.file-item').forEach(item => {
        ...
        function showCanvasPanel(path, content, mode) {
            ...
            if (window.innerWidth < 1024) {
                canvasPanel.classList.add('mobile-visible');
                if (resizer) {
                ...
                canvasPanel.classList.remove('mobile-visible');
                if (resizer) {
        ...
        function hideCanvasPanel() {
            ...
            canvasPanel.classList.remove('flex', 'mobile-visible');
            if (resizer) {
                resizer.classList.add('hidden');
            }
...
function createBotMessageContainer(animate = true) {
    ...
    const thinkingContainer = botMessageWrapper.querySelector('.thinking-process-container');
    ...
    thinkingContainer.dataset.expanded = 'false';
    thinkingContainer.dataset.initialized = 'false';
    thinkingContainer.dataset.answerStarted = 'false';
    thinkingContent.style.display = 'none';
    thinkingHeader.addEventListener('click', () => {
        const currentlyHidden = thinkingContent.style.display === 'none';
        const shouldShow = currentlyHidden;
        thinkingContent.style.display = shouldShow ? 'block' : 'none';
        thinkingContainer.dataset.expanded = shouldShow ? 'true' : 'false';
    });
...
function updateBotBubble(bubbleElement, responseContent, isFinal = false) {
    ...
    const thinkOpenIdx = responseContent.indexOf('<think>');
    const thinkCloseIdx = responseContent.indexOf('</think>');
    const hasThinkTag = thinkOpenIdx !== -1;

    let rawThinkContent = null;
    if (hasThinkTag) {
        if (thinkCloseIdx !== -1 && thinkCloseIdx > thinkOpenIdx) {
            rawThinkContent = responseContent.slice(thinkOpenIdx + 7, thinkCloseIdx);
        } else {
            rawThinkContent = responseContent.slice(thinkOpenIdx + 7);
        }
    }
    const trimmedThink = rawThinkContent ? rawThinkContent.trim() : '';

    let conversationalSection = '';
    if (hasThinkTag) {
        if (thinkCloseIdx !== -1 && thinkCloseIdx > thinkOpenIdx) {
            conversationalSection = responseContent.slice(0, thinkOpenIdx) + responseContent.slice(thinkCloseIdx + 8);
        } else {
            conversationalSection = responseContent.slice(0, thinkOpenIdx);
        }
    } else {
        conversationalSection = responseContent;
    }

    let conversationalContent = conversationalSection
    ...
    if (hasThinkTag) {
        ...
        if (thinkingContainer.dataset.initialized !== 'true') {
            thinkingContainer.dataset.initialized = 'true';
            thinkingContainer.dataset.expanded = 'true';
            thinkingContentEl.style.display = 'block';
        } else if (thinkingContainer.dataset.expanded === 'true') {
            thinkingContentEl.style.display = 'block';
        }

        if (trimmedThink) {
            thinkingContentEl.innerHTML = marked.parse(rawThinkContent);
        } else if (!thinkingContentEl.innerHTML.trim()) {
            thinkingContentEl.innerHTML = `<p class="text-sm text-gray-400 italic">Thinking...</p>`;
        }

        if (thinkingContainer.dataset.expanded === 'true') {
    ...
    if (conversationalContent) {
        ...
        if (thinkingContainer.style.display === 'block' && thinkingContainer.dataset.answerStarted !== 'true') {
            thinkingContentEl.style.display = 'none';
            thinkingContainer.dataset.expanded = 'false';
            thinkingContainer.dataset.answerStarted = 'true';
        }
    } else {
...
async function loadConversation(id) {
    ...
    hidePlanPanelFn = hidePlanPanel;
    hideCanvasPanelFn = hideCanvasPanel;
}
