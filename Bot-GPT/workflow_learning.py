...
def _verify_step(step: Dict[str, object], observed_text: str) -> bool:
    rule = step.get("verification_rule") or {}
    expected_text = str(rule.get("expected_text") or "").strip()
    if expected_text:
        return expected_text.lower() in observed_text.lower()
    return True
...
def replay_workflow(
    ...
    *,
    ...
    for step in steps:
        ...
        outcome = action(**(step.get("parameters") or {}))
