...
from tools import call_ollama_chat_stream, handle_tool_call, write_file
...
def initialize_chat(data):
    ...
    try:
        ...
        raise ValueError("Invalid 'messages' format") from exc
    ...
    conversation_id = data.get('conversation_id') or str(
        int(time.time() * 1000))
...
def stream_and_process_llm_response(model, messages, system_prompt):
    """Streams the LLM response and returns the full content."""
...
def process_tool_call(tool_match, conversation, messages):
    ...
    try:
        ...
        error_message = f"Error decoding tool JSON: {e}"
        messages.append(
...
def handle_ai_response(data):
    ...
    try:
        for _ in range(max_iterations):
            ...
            full_response_content = yield from stream_and_process_llm_response(
            ...
            if not tool_match:
                break
            yield from process_tool_call(tool_match, conversation, messages)
    finally:
...
def handle_chat_message(data):
    ...
    try:
        ...
        emit('ai_response', {
             "type": "agent_error", "error": "Invalid message payload"}, room=room)
        emit('ai_response', {"type": "done"}, room=room)
        return

    emit('ai_response', {"type": "user_message",
                         "content": last_user_message}, room=room, include_self=False)
    done_sent = False
    try:
        ...
        emit('ai_response', {
             "type": "agent_error", "error": str(exc)}, room=room)
    finally:
        if not done_sent:
            emit('ai_response', {"type": "done"}, room=room)
...
def process_final_answer(messages, conversation, canvas_mode):
    ...
    if canvas_mode:
        ...
        if code_block_match:
            ...
            file_extension = {'python': 'py', 'javascript': 'js', 'html': 'html',
                                'css': 'css', 'json': 'json', 'sql': 'sql',
                                'shell': 'sh', 'bash': 'sh'}.get(language, 'txt')
        else:
            content_to_save = re.sub(
                r'<think>[\s\S]*?<\/think>', '', final_answer_content).strip()
            file_extension = 'md'
        ...
        write_result = write_file(
            path=filename, content=content_to_save,
...
def save_conversation_history(conversation, messages):
    ...
    conversation_dir = os.path.join(
        current_app.config['USER_DATA_DIR'], str(
            conversation.owner_id), 'conversations'
    )
    os.makedirs(conversation_dir, exist_ok=True)
    conversation_path = os.path.join(conversation_dir, f"{conversation.id}.json")
    with open(conversation_path, 'w', encoding='utf-8') as f:
        json.dump({"messages": messages,
                  "title": conversation.title}, f, indent=2)
...
def handle_stop_agent(data):
    ...
    if conversation_id and conversation_id in AGENT_SESSIONS:
        ...
        emit('ai_response', {
             "type": "agent_error", "error": "Stop signal received. Attempting to halt..."})
