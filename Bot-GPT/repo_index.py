...
def _configured_repo_base_paths() -> List[str]:
    raw = current_app.config.get("REPO_SCAN_BASE_PATHS", [])
    ...
    if isinstance(raw, (list, tuple, set)):
        return [str(path) for path in raw if isinstance(path, str)]
    return []
...
def discover_local_repositories(
    ...
    roots = [path for path in (base_paths if base_paths is not None else _configured_repo_base_paths()) if isinstance(path, str) and os.path.isdir(path)]
    timeout_seconds = float(timeout_seconds or current_app.config.get("REPO_SCAN_TIMEOUT_SECONDS", 8.0))
    ...
    if not roots:
        return {
            ...
            "status": "blocked",
            "error": "I need a configured repo folder before I can scan local projects.",
        }
    discovered = []
    seen = set()
    ...
    for root in roots:
        for current_root, dirs, _files in os.walk(root):
            if (time.time() - started_at) > timeout_seconds:
                return {
                    "repos": rank_repositories(discovered),
                    "selected_repo_path": None,
                    "updated_at": time.time(),
                    "status": "error",
                    "error": "Repository discovery timed out before completion.",
                }
