...
def get_conversations():
    convos = [{"id": link.conversation.id, "title": link.conversation.title, "role": link.role} for link in current_user.conversations]
...
def get_conversation(session_id):
    ...
    convo_path = os.path.join(current_app.config['USER_DATA_DIR'], str(conversation.owner_id), 'conversations', f"{session_id}.json")
...
def share_conversation(session_id):
    ...
    new_participant = ConversationParticipant(user_id=user_id_to_share_with, conversation_id=session_id, role='participant')
...
def summarize_conversation(session_id):
    ...
    summary_prompt_messages = [{"role": msg["role"], "content": msg["content"]} for msg in messages if msg["role"] in ["user", "assistant"]]
