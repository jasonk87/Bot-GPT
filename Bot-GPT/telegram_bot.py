...
class TelegramBotBridge:
    ...
    def send_file(self, chat_id: int, file_path: str, caption: str = "") -> bool:
        method = "sendPhoto" if str(file_path).lower().endswith((".png", ".jpg", ".jpeg", ".webp", ".gif")) else "sendDocument"
        field = "photo" if method == "sendPhoto" else "document"
        try:
            with open(file_path, "rb") as handle:
                response = requests.post(
                    f"{self.base_url}/{method}",
                    data={"chat_id": int(chat_id), "caption": (caption or "")[:900]},
                    files={field: handle},
                    timeout=20,
                )
            return bool(response.ok)
        except Exception as exc:
            self.app.logger.warning("Telegram file send failed: %s", exc)
            return False
